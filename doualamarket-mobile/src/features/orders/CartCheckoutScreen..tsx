...
export default function CartCheckoutSheet({ items, total, onClose }: Props) {
  const { user } = useAuth();
        ...
        <View style={styles.sheet}>
          ...
          {stage === 'form' && (
            <>
              ...
              <Text style={styles.summary}>{items.length} item{items.length > 1 ? 's' : ''} · XAF {total.toLocaleString()}</Text>
              ...
              <View style={styles.methodRow}>
                ...
                >
                  <Text style={[styles.methodBtnText, paymentMethod === 'momo' && styles.methodBtnTextActive]}>MTN Mobile Money</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[styles.methodBtn, paymentMethod === 'om' && styles.methodBtnActive]}
                  onPress={() => setPaymentMethod('om')}
                >
                  <Text style={[styles.methodBtnText, paymentMethod === 'om' && styles.methodBtnTextActive]}>Orange Money</Text>
                </TouchableOpacity>
              </View>
...
const styles = StyleSheet.create({
  ...
  summary: { color: '#5B5F6B', marginBottom: 20, marginTop: 4 },
